...
export default function DropdownList(props) {
  ...
  const changeFocusedValueIndex = useCallback(
    (delta) => {
      setFocusedValueIndex((x) =>
        Math.min(Math.max(0, x + delta), values.length - 1),
      );
    },
    [values.length],
  );
  ...
  const mouseMove = (e, i) => {
    const userMoved =
      ...
      (mouseScreenPos.current.x !== e.screenX &&
        mouseScreenPos.current.y !== e.screenY);
  ...
  return (
    ...
    >
          ...
          return (
            <div
              class={
                ...
                (focusedValueIndex === i ? ' focused' : '')
              }
              onMouseMove={(e) => mouseMove(e, i)}
              onMouseEnter={
                mouseControl ? () => setFocusedValueIndex(i) : undefined
              }
              onMouseDown={(e) => {
                e.preventDefault();
                onValueSelected(v);
      ...
      {!values.length && (
        <div class="fjs-dropdownlist-empty">{emptyListMessage}</div>
      )}
    </div>
  );
}
