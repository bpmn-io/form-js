...
import {formFieldClasses, prefixId, sanitizeMultiSelectValue} from '../Util';
...
export default function Taglist(props) {
  const {disabled, errors = [], field, value: values = []} = props;
  ...
  const {state: loadState, values: options} = useValuesAsync(field);

  // We cache a map of option values to their index so that we don't need to search the whole options array every time to correlate the label
  const valueToOptionMap = useMemo(
    () =>
      Object.assign({}, ...options.map((o, x) => ({[o.value]: options[x]}))),
    [options],
  );
  ...
  return (
    <div class={formFieldClasses(type, errors)}>
      <Label label={label} id={prefixId(`${id}-search`, formId)} />
        ...
        {!disabled &&
          ...
          !isEscapeClosed && (
            <DropdownList
              ...
              emptyListMessage={
                hasOptionsLeft ? 'No results' : 'All values selected'
              }
