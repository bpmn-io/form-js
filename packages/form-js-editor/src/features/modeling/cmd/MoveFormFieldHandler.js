...
import {arrayAdd, arrayMove, arrayRemove, updatePath} from './Util';
...
export default class MoveFormFieldHandler {
  ...
  revert(context) {
    let {sourceFormField, targetFormField, sourceIndex, targetIndex} = context;
  ...
  moveFormField(context, revert) {
    ...
    if (sourceFormField.id === targetFormField.id) {
      ...
      get(schema, sourcePath).forEach((formField, index) =>
        updatePath(this._formFieldRegistry, formField, index),
      );
    } else {
      ...
      get(schema, targetPath).forEach((formField, index) =>
        updatePath(this._formFieldRegistry, formField, index),
      );
    }
